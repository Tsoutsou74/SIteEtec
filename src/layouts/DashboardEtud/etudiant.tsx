...
// ─── Composant Principal Étudiant ───────────────────────────
export default function EtudiantHome() {
  const { darkMode } = useTheme();
  const navigate = useNavigate();
  const [isLoading, setIsLoading] = useState(true);

  // ── États Dynamiques alimentés par l'API ──────────────────
  ...
  // Configuration des requêtes avec Token d'authentification
  ...
  useEffect(() => {
    const fetchHomeDashboardData = async () => {
      ...
      try {
        // Chargement des données globales d'accueil depuis ton ApiService
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-3 opacity-60 text-xs">
        ...
        <p className="font-bold">Chargement de votre espace académique...</p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl md:text-2xl font-black tracking-tight">
            Bonjour, {infosPerso.prenom || infosPerso.nom} 👋
          </h1>
          <p className="text-xs opacity-45 mt-1">
            {new Date().toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
            {' · '}{infosPerso.niveau} {infosPerso.mention} · {stats.semestre}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6">
        <StatCard icon={<TrendingUp size={18} />}    label="Moyenne générale"  value={stats.moyenne} sub={`↑ ${stats.evolutionMoyenne}`} color="#22c55e" />
        <StatCard icon={<BookOpen size={18} />}      label="Modules validés"   value={stats.modulesValides} sub={stats.semestre} color="#3b82f6" />
        <StatCard icon={<Clock size={18} />}         label="Heures de cours"   value={stats.heuresCours} sub="Ce sem." color="#f59e0b" />
        <StatCard icon={<ClipboardList size={18} />} label="Absences cumulées" value={stats.absences} sub="Justifiées" color="#8b5cf6" />
      </div>

      {/* Graphiques Dynamiques */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mt-6">
        <ChartCard title="Profil académique" subtitle="Visualisation des forces par matière">
          <ResponsiveContainer width="100%" height={220}>
            {notesGraphiques.length === 0 ? (
              <div className="h-full flex items-center justify-center text-[11px] opacity-40">Aucune donnée disponible</div>
            ) : (
        ...
        <ChartCard title="Notes détaillées" subtitle="Moyennes obtenues / 20">
          <ResponsiveContainer width="100%" height={220}>
            {notesGraphiques.length === 0 ? (
              <div className="h-full flex items-center justify-center text-[11px] opacity-40">Aucune donnée disponible</div>
            ) : (
        ...
        <ChartCard title="Évolution de la moyenne" subtitle="Suivi des performances par semestre">
          <ResponsiveContainer width="100%" height={220}>
            {historiqueSemestres.length === 0 ? (
              <div className="h-full flex items-center justify-center text-[11px] opacity-40">Aucun historique disponible</div>
            ) : (
      ...
      {/* Emploi du temps + Vigilances + Échéances */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mt-6">
        <div 
        ...
        >
          <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
            ...
            <button
              ...
              onClick={() => navigate('/etudiants/edt')}
            >
              Voir tout
            </button>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              ...
              <tbody>
                {edtSemaine.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center py-8 opacity-45">Aucun cours programmé cette semaine</td>
                  </tr>
                ) : (
        ...
        <div className="flex flex-col gap-4">
          {/* À surveiller (Filtre dynamique basé sur les notes reçues de l'API inférieures à 12/20) */}
          <div 
            className="rounded-2xl border overflow-hidden"
            style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)' }}
          >
            <div className="px-5 py-4 border-b" style={{ borderColor: 'var(--border)' }}>
              ...
              <p className="text-[11px] opacity-45 mt-0.5">Alertes et matières fragiles</p>
            </div>
            <ul className="divide-y" style={{ borderColor: 'var(--border)' }}>
              {notesGraphiques.filter(m => m.note < 12).length === 0 ? (
                <li className="p-4 text-center opacity-40 text-[11px]">Excellente situation, aucun module en danger</li>
              ) : (
          ...
          {/* Échéances administratives & académiques */}
          <div 
          ...
          >
            ...
            <ul className="divide-y" style={{ borderColor: 'var(--border)' }}>
              {echeances.length === 0 ? (
                <li className="p-4 text-center opacity-40 text-[11px]">Aucun événement à venir</li>
              ) : (
                echeances.map((e, i) => (
